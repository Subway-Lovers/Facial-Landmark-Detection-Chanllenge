...
def calculate_pitch_yaw_roll(landmarks_2D,
                             ...
                             cam_h=256,
                             radians=False):
    # 1. Set the parameters of camera
    # 2. Caculate the rotational vector/ translation vector, which can turn the 2D landmarks into 3D standardized landmarks
    # 3. Proceed the rotational vector/ translation vector to get the euler angles (that is, get pitch, yaw, roll)
    ...
    rmat, _ = cv2.Rodrigues(rvec) # Turn rotational vector into rotational matrix
    pose_mat = cv2.hconcat((rmat, tvec)) # horizontal concat
